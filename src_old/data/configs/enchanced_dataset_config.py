...
class EnhancedDatasetConfig:
    ...
    This config controls MIDI file loading, filtering, and preprocessing.
    Use TensorflowDatasetConfig to control tensor generation from the dataset.
    """
    ...
    # === Genre Filtering ===
    allowed_genres: Optional[List[str]] = None  # Filter by genre names (only include these)
    ...
    required_instruments: Optional[List[str]] = None  # MUST have these instruments

    # === Track Filtering ===
    min_tracks: int = 1  # Minimum number of tracks
    max_tracks: int = 16  # Maximum number of tracks (reduced from 128 for multitrack)
    min_notes: int = 10  # Minimum total notes in the piece
    min_notes_per_track: int = 1  # Minimum notes per track (filters sparse tracks)
    ...
    artist_folder_level: int = -2  # Folder level for artist (-2 = parent folder)

    # === Preprocessing Options ===
    resolution: int = 24  # Ticks per quarter note
    quantize: bool = True  # Quantize note timings
    remove_empty_tracks: bool = True  # Remove tracks with no notes
    merge_tracks_by_program: bool = False  # Merge tracks with same instrument

    # === Track Ordering ===
    # How to order tracks when max_tracks is reached
    track_priority: str = 'note_count'  # 'note_count', 'original', 'program'
    ...
    # === Valid Options ===
    VALID_TRACK_PRIORITIES: ClassVar[set[str]] = {'note_count', 'original', 'program'}
    ...
    def __post_init__(self):
        ...
        # Validate track priority
        if self.track_priority not in self.VALID_TRACK_PRIORITIES:
            raise ValueError(
                f"Invalid track_priority '{self.track_priority}'. "
                f"Must be one of: {', '.join(sorted(self.VALID_TRACK_PRIORITIES))}"
            )
    ...
    def load(cls, path: str) -> 'EnhancedDatasetConfig':
        ...
        defaults = {
            ...
            'merge_tracks_by_program': False,
            'track_priority': 'note_count',
        }

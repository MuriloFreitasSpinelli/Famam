...
def create_enhanced_dataset(config: EnhancedDatasetConfig) -> EnhancedMusicDataset:
    ...
    for midi_path in iterator:
        try:
            ...
            # Filter by allowed genres if specified
            if config.allowed_genres:
                if 'genre' not in metadata or metadata['genre'] not in config.allowed_genres:
                    genre_filtered += 1
                    filter_reasons['genre_not_allowed'] = filter_reasons.get('genre_not_allowed', 0) + 1
                    continue
    ...
    if config.verbose:
        ...
        if config.allowed_genres:
            print(f"  Filtered by genre: {genre_filtered}")

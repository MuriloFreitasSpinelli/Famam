...
class TrainingConfig:
    """
    Configuration for music generation model training with hyperparameter tuning support.

    Supports multiple architectures and representations:
    - LSTM/BiLSTM for sequential processing
    - CNN for local pattern detection
    - Transformer for attention-based modeling

    Representations supported (from muspy):
    - pianoroll: (128, time) or (tracks, 128, time) - best for polyphonic
    - pitch: (time,) - best for monophonic melodies
    - event: (events, 3) - event-based, compact
    - note: (notes, 4) - structured note representation
    """
    ...
    VALID_ARCHITECTURES: ClassVar[set[str]] = {
        'lstm', 'bilstm', 'cnn', 'cnn_lstm', 'transformer'
    }
    VALID_REPRESENTATIONS: ClassVar[set[str]] = {
        'pianoroll', 'pitch', 'event', 'note'
    }

    # ============ Model Architecture ============
    model_name: str
    architecture: str = 'lstm'  # 'lstm', 'bilstm', 'cnn', 'cnn_lstm', 'transformer'
    representation_type: str = 'pianoroll'  # Must match TensorflowDatasetConfig
    ...
    # CNN layers (for cnn/cnn_lstm architectures)
    ...
    # Transformer layers (for transformer architecture)
    transformer_heads: int = 4
    transformer_ff_dim: int = 128
    transformer_layers: int = 2

    # Multi-track options
    track_mode: str = 'single'  # 'single' or 'multitrack'
    max_tracks: int = 8
    track_embedding_dim: int = 16  # Embedding size for track/instrument info

    # Embedding dimensions
    embedding_dim: int = 32  # For genre/artist embeddings
    ...
    def __post_init__(self):
        ...
        if self.architecture.lower() not in self.VALID_ARCHITECTURES:
            raise ValueError(
                f"Invalid architecture '{self.architecture}'. "
                f"Must be one of: {', '.join(sorted(self.VALID_ARCHITECTURES))}"
            )

        # Validate representation type
        if self.representation_type.lower() not in self.VALID_REPRESENTATIONS:
            raise ValueError(
                f"Invalid representation_type '{self.representation_type}'. "
                f"Must be one of: {', '.join(sorted(self.VALID_REPRESENTATIONS))}"
            )

        # Set bidirectional based on architecture if using bilstm
        ...
        # Validate track mode
        if self.track_mode not in ('single', 'multitrack'):
            raise ValueError(
                f"Invalid track_mode '{self.track_mode}'. "
                f"Must be 'single' or 'multitrack'"
            )
    ...
    def summary(self) -> str:
        ...
        if self.track_mode == 'multitrack':
            summary_lines.append(f"  Max Tracks: {self.max_tracks}")

        if self.architecture in ('lstm', 'bilstm', 'cnn_lstm'):
        ...
        if self.architecture in ('cnn', 'cnn_lstm'):
            summary_lines.extend([
                f"  CNN Filters: {self.cnn_filters}",
                f"  CNN Kernel Sizes: {self.cnn_kernel_sizes}",
            ])

        if self.architecture == 'transformer':
            summary_lines.extend([
                f"  Transformer Heads: {self.transformer_heads}",
                f"  Transformer Layers: {self.transformer_layers}",
                f"  FF Dimension: {self.transformer_ff_dim}",
            ])

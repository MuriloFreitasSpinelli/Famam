...
def quantize_music(music: muspy.Music, grid: int = 1) -> muspy.Music:
    ...
    for track in music.tracks:
        for note in track.notes:
            ...
            # Quantize duration (minimum 1 tick)
...
def segment_music(
    ...
    if remainder > 0:
        padding_needed = segment_length - remainder
        padding_ratio = padding_needed / segment_length

        if padding_ratio <= max_padding_ratio:
            # Extract partial segment (it will be padded during tensor conversion)
            start_time = num_full_segments * segment_length
            segment = _extract_segment(music, start_time, start_time + remainder)
            segments.append(segment)
        # Otherwise, discard the partial segment
    ...
    for track in music.tracks:
        ...
        for note in track.notes:
            ...
            if start_time <= note.time < end_time:
                new_note = muspy.Note(
                    time=note.time - start_time,  # Shift to segment start
                    pitch=note.pitch,
...
def preprocess_music(
    ...
    This may produce multiple Music objects due to:
    - Segmentation (splits into chunks)
    ...
    music = adjust_resolution(music, config.target_resolution)

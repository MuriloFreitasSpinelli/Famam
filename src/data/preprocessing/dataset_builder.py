...
def build_dataset(config: MusicDatasetConfig) -> MusicDataset:
    """
    Build a MusicDataset from MIDI files according to config.

    Args:
        config: MusicDatasetConfig specifying paths, filters, and preprocessing
    ...
    if config.verbose:
        ...
        print(f"  Resolution: {config.resolution} ticks/beat")
        print(f"  Ticks per bar: {config.ticks_per_bar} (4/4 time)")
        print(f"  Quantize: {config.quantize} (grid={config.quantize_grid})")
        if config.segment_bars:
            print(f"  Segmentation: {config.segment_bars} bars ({config.segment_length} ticks) - BAR ALIGNED")
        elif config.segment_length:
            bars = config.segment_length / config.ticks_per_bar
            print(f"  Segmentation: {config.segment_length} ticks (~{bars:.1f} bars)")
        if config.enable_transposition:
            print(f"  Transposition: {config.transposition_semitones}")
        if config.enable_tempo_variation:
            print(f"  Tempo variation: {config.tempo_variation_range}")
    ...
    for midi_path in iterator:
        try:
            ...
            # Apply preprocessing
            processed_samples = preprocess_music(music, config)
    ...
    # Print summary
    if config.verbose:
        _print_build_summary(dataset, stats, config)
...
def _print_build_summary(
    ...
    if stats['files_processed'] > 0:
        expansion = stats['samples_added'] / stats['files_processed']
...
def build_and_save_dataset(config: MusicDatasetConfig) -> MusicDataset:
    """
    Build dataset and save to disk.

    Saves:
        - Dataset HDF5 file at config.output_path
        - Config JSON file alongside
...
def create_encoder_from_config(
    ...
    """
    Create the appropriate encoder based on config settings.

    Args:
        config: MusicDatasetConfig with encoder settings
        dataset: MusicDataset (needed for num_genres)

    Returns:
        BaseEncoder instance (EventEncoder or REMIEncoder)
    """
    from ..encoders import EventEncoder, REMIEncoder

    num_genres = dataset.vocabulary.num_genres

    if config.encoder_type == "event":
        return EventEncoder(
            num_genres=num_genres,
            num_instruments=129,
            encode_velocity=config.encode_velocity,
        )
    elif config.encoder_type == "remi":
        return REMIEncoder(
            num_genres=num_genres,
            num_instruments=129,
            resolution=config.resolution,
            positions_per_bar=config.positions_per_bar,
